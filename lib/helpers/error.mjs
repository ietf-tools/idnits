...
export class ValidationError extends Error {
   ...
   * @param {Object[]} [opts.lines] Array of lines and column positions
   * @param {string} [opts.path] Path where the error occured
   */
...
export function toContainError (received, expected, severity) {
  ...
  if (!Array.isArray(received)) {
    ...
    return {
      pass: false,
      message: () => `expected a result array with at least 1 error of type ${severity.name}`
    }
  }
