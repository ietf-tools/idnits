 ...
 * @param {string} [path] Current path array
 * @param {string} [key] Tag owning obj, when obj is a member of an array
 */
export async function traverseAll (obj, func, path = [], key = null) {
  const isAsyncFunc = func.constructor.name === 'AsyncFunction'
  if (typeof obj === 'string') {
    isAsyncFunc ? await func(obj, key, path) : func(obj, key, path)
  } else {
    for (const [key, value] of Object.entries(obj)) {
      ...
      if (Array.isArray(value)) {
        let idx = 0
        for (const entry of value) {
          await traverseAll(entry, func, [...path, `${key}[${idx}]`], key)
          idx++
        }
      } else if (isPlainObject(value)) {
 ...
 *
 * Repeated sibling tags are collapsed by the XML parser into an array, so a
 * string member of an array is the content of the tag named by `key` rather
 * than of an anonymous node. Callers decide whether a value is prose by its
 * key, so the owning tag is carried into the recursion.
 *
 * @param {Object} obj Object to traverse
 ...
 * @param {string} [key] Tag owning obj, when obj is a member of an array
 */
export async function traverseAllValues (obj, func, path = [], key = null) {
  const isAsyncFunc = func.constructor.name === 'AsyncFunction'
  if (typeof obj === 'string') {
    isAsyncFunc ? await func(obj, key, path) : func(obj, key, path)
  } else {
    for (const [key, value] of Object.entries(obj)) {
      if (typeof value === 'string') {
        ...
        let idx = 0
        for (const entry of value) {
          await traverseAllValues(entry, func, [...path, `${key}[${idx}]`], key)
          idx++
        }
      } else if (isPlainObject(value)) {
