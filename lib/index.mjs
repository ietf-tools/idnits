    ...
    case 'txt': {
      validations.push({
        ...
        tasks: [
          ...
          { key: 'txtcopyrightdate', title: 'Validating copyright date', task: async (ctx) => validateCopyrightDate(ctx.doc, ctx.options) },
          { key: 'txtlicensedeclarations', title: 'Validating license declarations', task: async (ctx) => validateLicenseDeclarations(ctx.doc, ctx.options) },
          { key: 'txtcopyrightsection', title: 'Validating copyright section', task: async (ctx) => validateCopyrightSection(ctx.doc, ctx.options) }
        ],
        concurrent: true
      })
    }
