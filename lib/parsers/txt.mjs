...
const licenseDeclarations = {
  trust_tlp4_6_b_i: /(Copyright\s+\(c\)\s+(\d{4})\s+IETF Trust and the persons identified as the\s+document authors\.\s*All rights reserved\.|Copyright\s+\(C\)\s+The Internet Society\s+\((\d{4})\)\.\s*All Rights Reserved\.)/gi,
  ...
  license6_c_i: /This document may not be modified, and derivative works of it may not be created, except to\s+format it for publication as an RFC or to translate it into languages other than English\./,
  license6_c_ii: /This document may not be modified, and derivative works of it may not be created, and it may not\s+be published except as an Internet-Draft\./
}
...
export async function parse (rawText, filename) {
  ...
  try {
    ...
    const match = [...normalizedText.matchAll(licenseDeclarations.trust_tlp4_6_b_i)]
    ...
    for (const line of rawText.split('\n')) {
      ...
      data.contains.copyrightSection6_b_i = licenseDeclarations.trust_tlp4_6_b_i.test(normalizedText)
