...
export async function parse (rawText, filename) {
  ...
  try {
    ...
    for (const line of rawText.split('\n')) {
      ...
      const domainMatches = [...trimmedLine.matchAll(FQDN2_RE)]
      if (domainMatches.length > 0) {
        domainMatches.forEach(match => {
          if (!isRestrictedFQDN(match.groups.domain) && !data.extractedElements.fqdnDomains.includes(match.groups.domain)) {
