...
const NON_RFC_REFERENCE_RE = /(?<=^|\s)\[(?!RFC\d+)[A-Za-z0-9.-]+\]/gi
...
export async function parse (rawText, filename) {
  ...
  try {
    ...
    const hyphenWords = collectHyphenWords(rawText)
    const allowedFrags = buildAllowedHyphenFrags(hyphenWords)
    data.possibleIssues.hyphenatedLines = findHyphenIssues(rawText, allowedFrags)
...
/**
 * Split text into words containing hyphens
 *
 * @param {string} rawText - The full document text
 * @returns {string[]} List of hyphenated words
 */
function collectHyphenWords (rawText) {
  return rawText
    .split(/[^A-Za-z0-9_-]+/)
    .filter(word =>
      word.includes('-') &&
...
/**
 * Build a set of all valid hyphen fragments from a list of words
 *
 * @param {string[]} words – Array of hyphenated words
 * @returns {Set<string>} Allowed fragments ending in a hyphen
 */
function buildAllowedHyphenFrags (words) {
  const frags = new Set()
...
/**
 * Identify real hyphenation breaks at end of lines, skipping URL splits
 *
 * @param {string} rawText – The full document text
 * @param {Set<string>} allowedFrags – Set of valid hyphen fragments
 * @returns {{ line: number; pos: number }[]} Array of issue objects
 */
function findHyphenIssues (rawText, allowedFrags) {
  const issues = []
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Skip continuation of a URL broken at previous line
    if (/\bhttps?:\/\/\S+-$/i.test(prevTrimmed) && trimmed.endsWith('-')) {
      prevTrimmed = trimmed
      continue
    }

    // Skip any other path/URL-like fragment ending with a hyphen
    if (/[./#]\S*-$/.test(trimmed)) {
      prevTrimmed = trimmed
      continue
    }

    // Check for genuine hyphen break
    if (trimmed.endsWith('-')) {
      const m = line.match(/([A-Za-z0-9_/.-]+-)$/)
      if (m && !allowedFrags.has(m[1])) {
        issues.push({ line: i + 1, pos: line.length })
      }
    }
