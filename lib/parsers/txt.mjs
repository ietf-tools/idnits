...
export async function parse (rawText, filename) {
  ...
  try {
    ...
    if (lastPageExpiration) {
      ...
      const parsed = DateTime.fromFormat(expiresStr, 'd LLLL yyyy', { zone: 'utc' }) ||
                     DateTime.fromFormat(expiresStr, 'LLLL d, yyyy', { zone: 'utc' })
            ...
            if (values.left.startsWith('Expires')) {
              ...
              if (dateValue) {
                if (dateValue.groups.iso) {
                  data.header.expires = DateTime.fromISO(dateValue.groups.iso, { zone: 'utc' })
                } else {
