...
export const FQDN_RE = /(?<=^|\s)(?<domain>(?:[a-z0-9-]+\.)+(?:[a-z]{2,}))\b/gi
export const FQDN2_RE = /(?<=^|\s)(?<domain>(?:[a-z0-9-]+\.){2,}[a-z]{2,})\b/gi
    ...
    case 'xml': {
      await traverseAllValues(doc.data, async (val, k, p) => {
        if (['t', '#text'].includes(k)) {
          const domainMatches = val.matchAll(FQDN2_RE)
          for (const match of domainMatches) {
            if (FALSE_POS_NUMS_RE.test(match.groups.domain) || isRestrictedFQDN(match.groups.domain)) {
