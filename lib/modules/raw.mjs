 ...
 */
export function validateContent (data, { mode = MODES.NORMAL } = {}) {
  const result = []
 ...
 * @returns {Array} List of errors/warnings/comments or empty if fully valid
 */
export function validateEncoding (raw, { mode = MODES.NORMAL } = {}) {
  const result = []
  ...
  if (detected.encoding === 'ascii') {
    ...
    if (mode === MODES.NORMAL) {
      result.push(new ValidationError('INVALID_ENCODING', `Input should be encoded using UTF-8, but detected ${detected.encoding}.`, { ref: 'https://datatracker.ietf.org/doc/html/rfc7997' }))
    } else if (mode === MODES.FORGIVE_CHECKLIST) {
      result.push(new ValidationWarning('INVALID_ENCODING', `Input should be encoded using UTF-8, but detected ${detected.encoding}.`, { ref: 'https://datatracker.ietf.org/doc/html/rfc7997' }))
    }
  }
 ...
 */
export function decodeBufferToUTF8 (raw) {
