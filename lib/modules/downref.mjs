    ...
    case 'xml': {
      ...
      const normativeReferencesSection = referencesSections.find(section => {
        if (typeof section.name === 'string') {
          return section.name?.toLowerCase().includes('normative references')
        } else if (isPlainObject(section.name)) {
          return section.name['#text']?.toLowerCase().includes('normative references')
        } else {
          return false
        }
      })
      ...
      const { drafts, rfcs } = refs.reduce((acc, ref) => {
      ...
      const normalizedReferences = [...draftList, ...rfcList]
