...
export async function validateDownrefs (doc, { mode = MODES.NORMAL } = {}) {
  ...
  switch (doc.type) {
    case 'txt': {
      ...
      const drafts = normalizeDraftReferencesForDownref(referenceSectionDraftReferences.filter((extracted) => extracted.subsection === 'normative_references').map((extracted) => extracted.value))
    ...
    case 'xml': {
      ...
      const normalizedReferences = normalizeXmlReferences(definedReferences)
