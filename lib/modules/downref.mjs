...
export async function validateDownrefs (doc, { mode = MODES.NORMAL } = {}) {
  ...
  switch (doc.type) {
    case 'txt': {
      const { referenceSectionRfc, draftStatusReferences } = doc.data.extractedElements
    ...
    case 'xml': {
      ...
      const normalizedReferences = extractReferencesFromXml(referencesSections, 'normative references')
...
/**
 * 
 * Extracts references from xml dcument references section
 * 
 * @param {Object} referencesSections - XML document references sections
 * @param {String} subsection - name of subsection we want to get references from
 * @returns {Array} - Array of references
 */
function extractReferencesFromXml (referencesSections, subsection) {
  const normativeReferencesSection = referencesSections.find(section => {
    if (typeof section.name === 'string') {
      return section.name?.toLowerCase().includes(subsection)
    } else if (isPlainObject(section.name)) {
      return section.name['#text']?.toLowerCase().includes(subsection)
    } else {
      return false
    }
  })

  const refs = normativeReferencesSection?.reference;

  if (!refs) { return [] }

  return getXmlReferences(refs)
}
 ...
 * @returns 
 */
function getXmlReferences (references) {
  ...
  const draftList = normalizeDraftReferences(Array.from(drafts));
...
export async function validateNormativeReferences (doc, { mode = MODES.NORMAL } = {}) {
  ...
  switch (doc.type) {
    ...
    case 'xml': {
      const referencesSections = doc.data.rfc?.back?.references?.references
      if (!referencesSections) { return result }
      const normativeReferences = extractReferencesFromXml(referencesSections, 'normative references')
