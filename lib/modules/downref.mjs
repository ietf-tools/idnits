...
import { isPlainObject, trimStart } from 'lodash-es'
...
export async function validateReferenceForStale (doc, { mode } = {}) {
  ...
  switch (doc.type) {
    case 'txt': {
      const referenceSectionDraftReferences = doc.data.extractedElements.draftStatusReferences
      const parsedDrafts = referenceSectionDraftReferences.map(r => {
        const { baseName, version } = splitDraft(r.value)
        return {
          baseName,
          version,
      ...
      for (let i = 0; i < parsedDrafts.length; i++) {
        const draftInfo = await fetchRemoteDocInfoJson(parsedDrafts[i].baseName)
        const lastVersion = draftInfo?.rev_history?.pop()
        if (!draftInfo || !draftInfo.rev_history || !lastVersion) {
          result.push(new ValidationWarning(
            'UNDEFINED_STATE',
            `The draft reference ${parsedDrafts[i].baseName} does not have a defined state or could not be fetched.`,
            { ref: `https://datatracker.ietf.org/doc/${parsedDrafts[i].baseName}` }
          ))
          continue
        } else {
          if (lastVersion.rev !== parsedDrafts[i].version) {
            result.push(new ValidationWarning(
              'OUTDATED_DRAFT',
              `The draft reference ${parsedDrafts[i].baseName} (version ${parsedDrafts[i].version}) is stale. The latest version is ${lastVersion.rev} for ${lastVersion.name}.`,
              { ref: `https://datatracker.ietf.org/doc/${parsedDrafts[i].baseName}` }
            ))
          }
        }
    ...
    case 'xml': {
      ...
      const parsedDrafts = drafts.map(r => {
        const { baseName, version } = splitDraft(r)
        return {
          baseName,
          version
        }
      ...
      for (let i = 0; i < parsedDrafts.length; i++) {
        ...
        if (!draftInfo || !draftInfo.rev_history || !lastVersion) {
          ...
          const { rev } = lastVersion
          if (rev !== parsedDrafts[i].version) {
            result.push(new ValidationWarning(
              'OUTDATED_DRAFT',
              `The draft reference ${parsedDrafts[i].baseName} is stale. The latest version is ${rev}.`,
              { ref: `https://datatracker.ietf.org/doc/${parsedDrafts[i].baseName}` }
            ))
          }
        }
