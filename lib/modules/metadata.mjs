    ...
    case 'txt': {
      ...
      if (!offline) {
        ...
        if (mode !== MODES.SUBMISSION && obsoletesRfc.length > 0) {
          const obsoletesFetchResults = await Promise.all(
            obsoletesRfc
              .filter(ref => RFC_NUM_RE_GENERAL.test(ref))
              .map(async ref => {
          ...
          result.push(...obsoletesFetchResults.filter(Boolean))
        }

        // -> Updates an already obsoleted rfc
        if (mode !== MODES.SUBMISSION && updatesRfc.length > 0) {
          const updatesFetchResults = await Promise.all(
            updatesRfc
              .filter(ref => RFC_NUM_RE_GENERAL.test(ref))
              .map(async ref => {
                const rfcInfo = await fetchRemoteRfcInfo(ref)
          ...
          result.push(...updatesFetchResults.filter(Boolean))
        }
      }
    ...
    case 'xml': {
      ...
      if (!offline) {
        ...
        if (mode !== MODES.SUBMISSION && obsoletesRef.length > 0) {
          const obsoletesFetchResults = await Promise.all(
            obsoletesRef
              .filter(ref => RFC_NUM_RE.test(ref))
              .map(async ref => {
                const rfcInfo = await fetchRemoteRfcInfo(ref)
                if (!rfcInfo) {
                  return new ValidationWarning(
