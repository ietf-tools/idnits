  ...
  // -> Check for existing doc stream mismatch
    ...
    if (docInfo) {
      ...
      if (!existingStream && ['ietf', 'iab', 'irtf'].includes(submissionType)) {
        result.push(new ValidationWarning('SUBMISSION_TYPE_UNEXPECTED', 'A document stream is specified in the rfc tag but the existing version has no stream on Datatracker. Is this intentional?', {
          ref: 'https://www.rfc-editor.org/rfc/rfc7841.html',
          path: 'rfc.submissionType'
        }))
      // -> Existing stream doesn't match submission type
        ...
        result.push(new ValidationWarning('SUBMISSION_TYPE_UNEXPECTED', 'The document stream specified in the rfc tag does not match the existing version on Datatracker.', {
          ref: 'https://www.rfc-editor.org/rfc/rfc7841.html',
          path: 'rfc.submissionType'
        }))
      }
