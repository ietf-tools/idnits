...
import { find, findIndex, get, has, isPlainObject } from 'lodash-es'
...
export async function validateAbstractSection (doc, { mode = MODES.NORMAL } = {}) {
  ...
  switch (doc.type) {
    case 'txt': {
      ...
      } else if (!doc.data.content.abstract || doc.data.content.abstract?.length < MIN_SECTION_CONTENT_LENGTH) {
  ...
  switch (doc.type) {
    case 'txt': {
      ...
      if (!doc.data.markers.introduction.start) {
        ...
        if (doc.data.content.introduction?.length < MIN_SECTION_CONTENT_LENGTH) {
          result.push(
            new ValidationError(
      ...
      if (!introSection) {
        ...
        if (childrenTypes.length < 1) {
          result.push(new ValidationError('INVALID_INTRODUCTION_SECTION', 'The introduction section is empty.', {
            ref: 'https://authors.ietf.org/en/required-content#introduction',
            path: `rfc.middle.section[name=${introSection.name}]`
          }))
        } else {
  ...
  switch (doc.type) {
    case 'txt': {
      if (!doc.data.markers.securityConsiderations.start) {
        ...
        if (doc.data.content.securityConsiderations?.length < MIN_SECTION_CONTENT_LENGTH) {
          result.push(
            new ValidationError(
  ...
  switch (doc.type) {
    case 'txt': {
      ...
      if (!doc.data.markers.references.start) {
        ...
        break
      } else if (referencesContent?.length < MIN_SECTION_CONTENT_LENGTH) {
        switch (mode) {
          case MODES.NORMAL: {
            result.push(
              new ValidationError(
      ...
      const subsections = []
      const plainReferenceFormat = /^(?:\d+\.\s+)?References$/i
      const referenceSectionTitlePattern = /^(?:\d+\.\s+)?(?:(?:Normative|Informative)\s+)?References$/i
      let numberOfReferenceSectionTitles = 0

      for (const line of referencesContent) {
        const match = subsectionPattern.test(line)
        if (match) {
          subsections.push(line)
        } else if (referenceSectionTitlePattern.test(line)) {
          numberOfReferenceSectionTitles++
        }
      }

      if (subsections.length === 0 && plainReferenceFormat.test(referencesContent[0])) {
        switch (mode) {
          case MODES.NORMAL: {
            result.push(
              new ValidationError(
      ...
      if (numberOfReferenceSectionTitles > 1) {
        result.push(
          new ValidationError(
            'MULTIPLE_REFERENCES_SECTION_TITLES',
            'Multiple occurrences of the References section title detected. There should only be one top-level References section.',
            {
              ref: 'https://authors.ietf.org/required-content#references'
            }
          )
    ...
    case 'xml': {
      ...
      if (refsSections?.length > 0) {
        ...
        for (const refs of refsSections) {
          if (refs._attr?.title && mode !== MODES.SUBMISSION) {
            result.push(new ValidationWarning('DEPRECATED_REFERENCES_TITLE', 'The references section should have a <name> child element instead of a title attribute.', {
              ref: 'https://authors.ietf.org/required-content#references',
          ...
          if (!refs.name) {
            switch (mode) {
              case MODES.NORMAL: {
                result.push(new ValidationError('MISSING_REFERENCES_NAME', 'The references section is missing a <name> child element.', {
                  ref: 'https://authors.ietf.org/required-content#references',
                  path: `rfc.back.references[${idx}].name`
                }))
                break
              }
              case MODES.FORGIVE_CHECKLIST: {
                result.push(new ValidationWarning('MISSING_REFERENCES_NAME', 'The references section is missing a <name> child element.', {
                  ref: 'https://authors.ietf.org/required-content#references',
                  path: `rfc.back.references[${idx}].name`
                }))
                break
          ...
          const nameValue = ((typeof refs.name === 'string') ? refs.name : refs.name['#text'] ?? '').toLowerCase()
          if (!(nameValue.indexOf('informative') >= 0 || nameValue.indexOf('normative') >= 0)) {
            switch (mode) {
              case MODES.NORMAL: {
                result.push(new ValidationError('INVALID_REFERENCES_NAME', 'The references section <name> element should be either Normative or Informative.', {
                  ref: 'https://authors.ietf.org/required-content#references',
                  path: `rfc.back.references[${idx}].name`
                }))
                break
              ...
              case MODES.FORGIVE_CHECKLIST: {
                result.push(new ValidationWarning('INVALID_REFERENCES_NAME', 'The references section <name> element should be either Normative or Informative.', {
                  ref: 'https://authors.ietf.org/required-content#references',
                  path: `rfc.back.references[${idx}].name`
                }))
                break
...
export async function validateIANAConsiderationsSection (doc, { mode = MODES.NORMAL } = {}) {
  ...
  switch (doc.type) {
    ...
    case 'xml': {
      const sections = get(doc, 'data.rfc.middle.section', [])
      const secSectionIdx = findIndex(sections, s => s.name === 'IANA Considerations')
      const secSection = secSectionIdx >= 0 ? sections[secSectionIdx] : null
      if (!secSection) {
        if (doc.docKind === 'rfc') {
          if (mode === MODES.NORMAL || mode === MODES.FORGIVE_CHECKLIST) {
            result.push(new ValidationComment('MISSING_IANA_CONSIDERATIONS_SECTION', 'The IANA considerations section is missing.', {
        ...
        if (childrenTypes.length < 1) {
          result.push(new ValidationError('INVALID_IANA_CONSIDERATIONS_SECTION', 'The IANA considerations section is empty.', {
            path: `rfc.middle.section[${secSectionIdx}]`,
            ref: 'https://authors.ietf.org/en/required-content#iana-considerations'
          }))
        } else {
          for (const key of childrenTypes) {
  ...
  switch (doc.type) {
    case 'txt': {
      const declaredReferences = [
        ...doc.data.extractedElements.referenceSectionRfc.filter((el) => el.subsection !== 'appendix').map((el) => el.value),
        ...doc.data.extractedElements.referenceSectionDraftReferences.filter((el) => el.subsection !== 'appendix').map((el) => el.value)
      ]
    ...
    case 'xml': {
      ...
      if (Array.isArray(referencesSections)) {
        for (const section of referencesSections) {
          if (Array.isArray(section.reference)) {
            for (const ref of section.reference) {
