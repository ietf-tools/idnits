  ...
  switch (doc.type) {
    ...
    case 'xml': {
      ...
      if (!introSection) {
        switch (mode) {
          case MODES.NORMAL: {
            result.push(new ValidationError('MISSING_INTRODUCTION_SECTION', 'The introduction section is missing.', { ref: 'https://authors.ietf.org/en/required-content#introduction' }))
            break
          }
          case MODES.FORGIVE_CHECKLIST: {
            result.push(new ValidationWarning('MISSING_INTRODUCTION_SECTION', 'The introduction section is missing.', { ref: 'https://authors.ietf.org/en/required-content#introduction' }))
            break
          }
        }
      } else {
        ...
        if (childrenTypes.length < 1) {
          result.push(new ValidationError('INVALID_INTRODUCTION_SECTION', 'The introduction section is empty.', { ref: 'https://authors.ietf.org/en/required-content#introduction' }))
        } else {
          for (const key of childrenTypes) {
            if (!['artwork', 'aside', 'blockquote', 'dl', 'figure', 'iref', 'ol', 'sourcecode', 't', 'table', 'texttable', 'ul'].includes(key)) {
              result.push(new ValidationError('INVALID_INTRODUCTION_SECTION_CHILD', 'The introduction section must consist of <artwork>, <aside>, <blockquote>, <dl>, <figure>, <iref>, <ol>, <sourcecode>, <t>, <table>, <texttable> or <ul> elements only.', { ref: 'https://www.rfc-editor.org/rfc/rfc7991.html#section-2.46' }))
              break
            }
          }
        }
